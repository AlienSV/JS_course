let renderer = {
    renderBoard() {
        let result = this.generateBoard();
        document.body.insertAdjacentHTML('afterbegin', result);
        this.clearUserStartingSpot(); //очистка стартовой области от препятствий чтобы точно было куда ходить
        this.renderUserPosition(player);
    },
    //препятствия вставляются на поле с вероятностью около 20% добавлением имени класса "obstacle" к атрибутам ячейки
    generateBoard() {
        let board = '';
        for (let y = 0; y < config.rowsCount; y++) {
            board += '<tr>';
            for (let x = 0; x < config.colsCount; x++) {
                board += `<td data-x="${x}" data-y="${y}" class="${Math.random() <= 0.2 ? "obstacle" : ""}"></td>`;
            }
            board += '</tr>';
        }
        return `<table><tbody>${board}</tbody></table>`
    },

    getSquare(position) {
        return document.querySelector(`[data-x="${position.x}"][data-y="${position.y}"]`);

    },

    renderUserPosition(player) {
        let square = this.getSquare(player);
        square.classList.add('user');
    },

    clearUserPosition() {
        document.querySelector('.user').classList.remove('user');
    },

    clearUserStartingSpot() {
        for (let i = 0; i < 2; i++)
            for (j = 0; j < 2; j++) {
                this.getSquare({
                    x: i,
                    y: j
                }).className = "";
            }
    }
};
